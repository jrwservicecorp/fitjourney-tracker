/* script.js - FitJourney Tracker - Modern Edition - JS v3.6.3 */
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function openComparisonEditor() {
    ...
    const beforePhoto = photoLogs[beforeIndex];
    const afterPhoto = photoLogs[afterIndex];
    // Use DOM element instead of jQuery to set display style
    ...
    Konva.Image.fromURL(beforePhoto.src, function(img) {
      // Compute scale to fit within 350x400 area
      const originalWidth = img.width();
      const originalHeight = img.height();
      let scale = Math.min(350 / originalWidth, 400 / originalHeight);
      img.setAttrs({
        x: 0,
        y: 0,
        scaleX: scale,
